...
class BookManagerApp(QWidget):
    ...
    def get_book_info(self, barcode):
        """바코드로 도서 정보 반환 (dict 또는 None) - O(1) 조회"""
    ...
    def get_overdue_count(self):
        ...
        for info in self.loan_status.values():
            days = (datetime.now() - info["date"]).days
    ...
    def check_student(self):
        ...
        for b, info in self.loan_status.items():
            if info["name"] == student_id:
                ...
                days = (datetime.now() - info["date"]).days
                if days > self.OVERDUE_DAYS:
                    overdue_list.append(f"{title} ({days}일 연체)")
    ...
    def _create_return_button(self, dialog, table, barcode, student_name, update_title_func=None):
        ...
        def do_return():
            ...
            try:
                ...
                # 바코드로 현재 행을 다시 찾아 제거 (인덱스 밀림 문제 완전 해결)
    ...
    def show_loan_status(self):
        ...
        for barcode, info in list(self.loan_status.items()):
            ...
            status = f"{days}일 경과"
            if days > self.OVERDUE_DAYS:
                status = f"⚠️ 연체 {days - self.OVERDUE_DAYS}일"

            rows_data.append([barcode, title, author, ar_level, info["name"], info["date"].strftime('%Y-%m-%d'), status])
    ...
    def show_overdue_list(self):
        ...
        for barcode, info in self.loan_status.items():
            days = (datetime.now() - info["date"]).days
            if days > self.OVERDUE_DAYS:
                overdue_items.append((barcode, info, days))
        ...
        table = QTableWidget()
        table.setColumnCount(8)
        table.setHorizontalHeaderLabels(["바코드", "제목", "저자", "AR Level", "대출자", "대출일", "연체일수", "반납"])
        table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        table.horizontalHeader().setSectionResizeMode(7, QHeaderView.ResizeMode.Fixed)
        table.setColumnWidth(7, 90)
        table.setSortingEnabled(False)
        ...
        for i, (barcode, info, days) in enumerate(overdue_items):
            book = self.get_book_info(barcode)
            if book:
                title = book.get('Title', '')
                author = book.get('Author', '')
            ...
            row_data = [
                ...
                info["name"], info["date"].strftime('%Y-%m-%d'),
                f"{days - self.OVERDUE_DAYS}일 연체"
            ]
    ...
    def export_loan_status(self):
        ...
        try:
            ...
            for row_idx, (barcode, info) in enumerate(self.loan_status.items(), 2):
                ...
                status = f"{days}일 경과"
                if days > self.OVERDUE_DAYS:
                    status = f"연체 {days - self.OVERDUE_DAYS}일"
                ...
                ws.cell(row=row_idx, column=6, value=info["date"].strftime('%Y-%m-%d'))
